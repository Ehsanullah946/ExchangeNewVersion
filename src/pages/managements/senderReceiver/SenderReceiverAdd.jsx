...
import { RiSendPlaneLine, RiUserAddFill } from 'react-icons/ri';

import { Link, useNavigate } from 'react-router-dom';
import { useCreateSenderReceiver } from '../../../hooks/useSenderReceiver';
import { useToast } from '../../../hooks/useToast';
const SenderReceiverAdd = () => {
  ...
  const { mutate, isLoading, error } = useCreateSenderReceiver();
  ...
  const handleSubmit = (e) => {
    ...
    mutate(cleanData, {
      onSuccess: () => {
        toast.success(t('senderReceveiver Created'));
        setTimeout(() => navigate('/management/senderReceiverList'), 1000);
      },
      onError: (error) => {
        console.error('Backend error:', error);
        let errorMessage = t('createSenderReceiverFaild');

        if (error.response?.data?.message) {
          errorMessage = error.response.data.message;

          if (error.response.data.message.includes('Validation error')) {
            errorMessage = t('SenderReceiverDuplicate');
      ...
      <div className="grid justify-center">
        <div className=" flex mt-1 justify-between  gap-1 bg-white/80 backdrop-blur-sm rounded-2xl shadow-lg border border-white/20 mb-1">
          <div className="flex flex-wrap justify-center items-center p-2 gap-3">
            <Link to="/management/senderReceiverList">
...
export default SenderReceiverAdd;
