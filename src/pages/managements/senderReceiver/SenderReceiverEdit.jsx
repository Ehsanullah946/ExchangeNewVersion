...
const SenderReceiverEdit = () => {
  const { id } = useParams();
  const { t } = useTranslation();
  const navigate = useNavigate();
  const toast = useToast();

  const { data, isLoading: loadingSenderReceiver } =
    useSingleSenderReceiver(id);
  ...
  useEffect(() => {
    if (data?.data) {
      const senderReceiver = data.data;
      const stakeholder = senderReceiver.Stakeholder || {};
    ...
    updateSenderReceiver(
      { id, payload: cleanData },
      {
        onSuccess: () => {
          ...
          navigate('/management/senderReceiverList');
        },
        onError: (err) => {
          console.error(err);
          ...
          <form onSubmit={handleSubmit}>
            ...
            <div className="grid sm:grid-cols-2 gap-6 p-3 rounded-b-2xl ltr:mr-4 rtl:ml-4 px-4 md:px-6 lg:px-10 border-b-2 border-t-2 shadow-2xl max-w-7xl mx-auto">
              ...
              <div className="flex flex-wrap justify-center sm:justify-start gap-2 col-span-full">
                ...
                <Link to="/management/senderRecevierList">
                  <button
                    type="button"
...
export default SenderReceiverEdit;
